...
[visit Leetcode link above for visuals]

Example 1:
Input: head = [4,2,8], root = [1,4,4,null,2,2,null,1,null,6,8,null,null,
  null,null,1,3]
Output: true
...
const isSubPath = (head, root) => {
  ...
  if (treeToListCheck(head, root)) return true;
  return isSubPath(head, root.right) || isSubPath(head, root.left);
};

const treeToListCheck = (head, root) => {
  if (!head) return true;
  if (!root) return false;
  if (head.val !== root.val) return false;
  return treeToListCheck(head.next, root.left) || treeToListCheck(head.next, root.right);
};